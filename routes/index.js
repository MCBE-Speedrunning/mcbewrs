...
router.get("/leaderboard/:cat?", function (req, res, next) {
	...
	if (typeof(req.params.cat) === "undefined") {
        ...
        db.all("SELECT date, readable, time, name, nationality FROM runners, runs, pairs, categories WHERE runners.rowid = runner_id AND runs.rowid = run_id AND abbreviation = category ORDER BY date DESC LIMIT 10", [], function (err, recent) {
