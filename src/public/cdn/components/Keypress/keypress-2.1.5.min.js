...
(function () {
	...
	var g = function (a, c) {
		...
		for (b in r)
			D.call(r, b) &&
				((d = r[b]), (this._defaults[b] = this._defaults[b] || d));
	...
	g.prototype.destroy = function () {
		...
		a = function (a, b, d) {
			if (null != a.removeEventListener)
				return a.removeEventListener(b, d);
	...
	g.prototype._bug_catcher = function (a) {
		...
		if (
			"cmd" === k &&
			0 <= j.call(this._keys_down, "cmd") &&
			"cmd" !== (c = y(null != (b = a.keyCode) ? b : a.key)) &&
			"shift" !== c &&
			"alt" !== c &&
			"caps" !== c &&
			"tab" !== c
	...
	g.prototype._prevent_default = function (a, c) {
		if (
			(c || this.should_suppress_event_defaults) &&
	...
	g.prototype._add_to_active_combos = function (a) {
		...
		if (this._active_combos.length) {
			e = i = 0;
			for (
				l = this._active_combos.length;
				0 <= l ? i < l : i > l;
				e = 0 <= l ? ++i : --i
			)
				if (
					(c = this._active_combos[e]) &&
					c.is_exclusive &&
					a.is_exclusive
				) {
					c = c.keys;
					...
					h &&
						(d
							? ((c = this._active_combos.splice(e, 1)[0]),
							  null != c && c.reset())
							: ((c = this._active_combos.splice(e, 1, a)[0]),
							  null != c && c.reset(),
							  (d = !0)),
						(f = !1));
				}
	...
	g.prototype._get_possible_sequences = function () {
		...
		for (g = n.length; f < g; f++) {
			...
			c = h = 1;
			for (
				l = this._sequence.length;
				1 <= l ? h <= l : h >= l;
				c = 1 <= l ? ++h : --h
			)
				if (((e = this._sequence.slice(-c)), a.is_sequence)) {
					if (
						0 > j.call(a.keys, "shift") &&
						((e = v(e, function (a) {
							return "shift" !== a;
						})),
						!e.length)
	...
	g.prototype._get_sequence = function (a) {
		...
		for (n = l.length; h < n; h++)
			if (((c = l[h]), c.is_sequence)) {
				b = i = 1;
				for (
					m = this._sequence.length;
					1 <= m ? i <= m : i >= m;
					b = 1 <= m ? ++i : --i
				)
					if (
						((f = v(this._sequence, function (a) {
							return 0 <= j.call(c.keys, "shift")
								? !0
								: "shift" !== a;
						}).slice(-b)),
						...
						for (
							...
							if (
								((e = f[b]),
								!(
									0 > j.call(c.keys, "shift") && "shift" === e
								) &&
									!(
										"shift" === a &&
										0 > j.call(c.keys, "shift")
									))
							)
								if (c.keys[b] === e) d = !0;
								else {
	...
	g.prototype._receive_input = function (a, c) {
		...
		if (this._prevent_capture)
			...
			((b = y(null != (d = a.keyCode) ? d : a.key)),
			(c || this._keys_down.length || !("alt" === b || b === k)) && b)
		)
			return c ? this._key_down(b, a) : this._key_up(b, a);
	};
	g.prototype._fire = function (a, c, b, d) {
		"function" === typeof c["on_" + a] &&
			this._prevent_default(
				b,
				!0 !== c["on_" + a].call(c["this"], b, c.count, d)
			);
		"release" === a && (c.count = 0);
	...
	g.prototype._match_combo_arrays = function (a, c) {
		...
		for (f = h.length; e < f; e++)
			(d = h[e]),
				(b = a.slice(0)),
				d.normalize_caps_lock &&
					0 <= j.call(b, "caps") &&
					b.splice(b.indexOf("caps"), 1),
				((!d.is_unordered && x(b, d.keys)) ||
					(d.is_unordered && u(b, d.keys))) &&
					c(d);
	};
	g.prototype._fuzzy_match_combo_arrays = function (a, c) {
		...
		for (e = f.length; d < e; d++)
			(b = f[d]),
				((!b.is_unordered && B(b.keys, a)) ||
					(b.is_unordered && E(b.keys, a))) &&
					c(b);
	};
	...
	g.prototype._handle_combo_down = function (a, c, b, d) {
		...
		e = !1;
		if (
			0 <= j.call(this._keys_down, b) &&
			((e = !0), !a.allows_key_repeat())
		)
			return !1;
		...
		if (
			...
			return this._fire("keydown", a, d, e);
	};
	g.prototype._key_up = function (a, c) {
		...
		e = s[b];
		c.shiftKey
			? (e && 0 <= j.call(this._keys_down, e)) || (a = b)
			: (b && 0 <= j.call(this._keys_down, b)) || (a = e);
		(f = this._get_sequence(a)) && this._fire("keyup", f, c);
		if (0 > j.call(this._keys_down, a)) return !1;
		f = h = 0;
		for (
			g = this._keys_down.length;
			0 <= g ? h < g : h > g;
			f = 0 <= g ? ++h : --h
		)
	...
	g.prototype._handle_combo_up = function (a, c, b) {
		...
		if (
			!a.keyup_fired &&
			((d = this._keys_down.slice()),
			d.push(b),
			!a.is_solitary || u(d, a.keys))
		)
			this._fire("keyup", a, c),
				a.is_counting &&
	...
	g.prototype.unregister_combo = function (a) {
		...
		for (e = f.length; d < e; d++)
			(c = f[d]),
				null != c &&
					((c.is_unordered && u(a, c.keys)) ||
					(!c.is_unordered && x(a, c.keys))
						? g.push(b(c))
						: g.push(void 0));
		return g;
	};
	...
	H = function (a) {
		...
		for (g = i.length; c < g; c++)
			(d = i[c]),
				F(d) || (p('Do not recognize the key "' + d + '"'), (f = !1));
