...
(function () {
	...
	keypress.Listener = (function () {
		function Listener(element, defaults) {
			...
			if (
				...
				if (element.length !== 1) {
					_log_error(
						"Warning: your jQuery selector should have exactly one object."
					);
				}
				element = element[0];
			}
		...
		Listener.prototype._bug_catcher = function (e) {
			...
			if (
				...
				(_ref = _convert_key_to_readable(
					(_ref1 = e.keyCode) != null ? _ref1 : e.key
				)) !== "cmd" &&
				_ref !== "shift" &&
				_ref !== "alt" &&
				_ref !== "caps" &&
		...
		Listener.prototype._cmd_bug_check = function (combo_keys) {
			if (
				_metakey === "cmd" &&
				__indexOf.call(this._keys_down, "cmd") >= 0 &&
				__indexOf.call(combo_keys, "cmd") < 0
			) {
				return false;
			}
			return true;
		...
		Listener.prototype._prevent_default = function (e, should_prevent) {
			if (
				(should_prevent || this.should_suppress_event_defaults) &&
				!this.should_force_event_defaults
			) {
				if (e.preventDefault) {
					e.preventDefault();
				} else {
		...
		Listener.prototype._get_potential_combos = function (key) {
			...
			for (_i = 0, _len = _ref.length; _i < _len; _i++) {
				...
				if (
					__indexOf.call(combo.keys, key) >= 0 &&
					this._cmd_bug_check(combo.keys)
				) {
		...
		Listener.prototype._add_to_active_combos = function (combo) {
			...
			if (__indexOf.call(this._active_combos, combo) >= 0) {
				...
				for (
					i = _i = 0, _ref = this._active_combos.length;
		...
		Listener.prototype._get_possible_sequences = function () {
			...
			for (_i = 0, _len = _ref.length; _i < _len; _i++) {
				combo = _ref[_i];
				for (
					j = _j = 1, _ref1 = this._sequence.length;
		...
		Listener.prototype._fire = function (
			event,
			combo,
			key_event,
			is_autorepeat
			...
			if (typeof combo["on_" + event] === "function") {
				this._prevent_default(
					key_event,
					combo["on_" + event].call(
						combo["this"],
						key_event,
						combo.count,
						is_autorepeat
		...
		Listener.prototype._match_combo_arrays = function (
			potential_match,
			match_handler
		) {
			...
			for (_i = 0, _len = _ref.length; _i < _len; _i++) {
				...
				if (
					source_combo.normalize_caps_lock &&
					__indexOf.call(combo_potential_match, "caps") >= 0
				) {
					combo_potential_match.splice(
						combo_potential_match.indexOf("caps"),
						1
					);
				}
				if (
		...
		Listener.prototype._fuzzy_match_combo_arrays = function (
			potential_match,
			match_handler
		) {
		...
		Listener.prototype._key_down = function (key, e) {
			...
			for (mod in _modifier_event_mapping) {
				...
				if (
					...
					for (
						i = _i = 0, _ref = this._keys_down.length;
						0 <= _ref ? _i < _ref : _i > _ref;
						i = 0 <= _ref ? ++_i : --_i
					) {
		...
		Listener.prototype._handle_combo_down = function (
			combo,
			potential_combos,
			key,
			e
			...
			if (combo.is_exclusive) {
				for (_i = 0, _len = potential_combos.length; _i < _len; _i++) {
					...
					if (
						potential_combo.is_exclusive &&
						potential_combo.keys.length > combo.keys.length
					) {
		...
		Listener.prototype._key_up = function (key, e) {
			...
			if (active_combos_length > 1) {
				...
				for (_l = 0, _len2 = _ref3.length; _l < _len2; _l++) {
					...
					if (
						active_combo === void 0 ||
						__indexOf.call(combos, active_combo) >= 0
					) {
						continue;
					}
		...
		Listener.prototype._handle_combo_up = function (combo, e, key) {
			...
			if (!combo.keyup_fired) {
				...
				if (
					...
					if (
						combo.is_counting &&
						typeof combo.on_keyup === "function" &&
						typeof combo.on_keydown !== "function"
					) {
						combo.count += 1;
					}
		...
		Listener.prototype.unregister_combo = function (keys_or_combo) {
			...
			unregister_combo = (function (_this) {
				return function (combo) {
					...
					_results = [];
					for (
						i = _i = 0, _ref = _this._registered_combos.length;
						0 <= _ref ? _i < _ref : _i > _ref;
						i = 0 <= _ref ? ++_i : --_i
					) {
			...
			if (keys_or_combo instanceof Combo) {
				...
				if (typeof keys_or_combo === "string") {
					...
					for (
						i = _i = 0, _ref = keys_or_combo.length;
						0 <= _ref ? _i < _ref : _i > _ref;
						i = 0 <= _ref ? ++_i : --_i
					) {
				...
				for (_j = 0, _len = _ref1.length; _j < _len; _j++) {
					...
					if (
						(combo.is_unordered &&
							_compare_arrays(keys_or_combo, combo.keys)) ||
						(!combo.is_unordered &&
							_compare_arrays_sorted(keys_or_combo, combo.keys))
					) {
	...
	_validate_combo = function (combo) {
		...
		if (
			__indexOf.call(combo.keys, "meta") >= 0 ||
			__indexOf.call(combo.keys, "cmd") >= 0
		) {
		...
		for (property in combo) {
			...
			if (_factory_defaults[property] === "undefined") {
				_log_error(
					"The property " +
						property +
						" is not a valid combo property. Your combo has still been registered."
				);
			}
		}
		return validated;
