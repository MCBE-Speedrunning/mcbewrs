...
export default async function renderSass(req: Request, res: Response, next: NextFunction) {
	// Ignore non-CSS requests
	if (!req.path.endsWith(".css"))
		return next();

	// Derive SCSS filepath from CSS request path
	const file = path.join(process.cwd(), "public", "cdn", req.path).replace(".css", ".scss");
